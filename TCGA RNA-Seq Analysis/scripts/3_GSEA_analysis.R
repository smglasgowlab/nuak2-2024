...
gene.list <- data.frame("Gene ID" = rownames(res.df),
                        "Rank" = res.df$t)
...
gene.list.KEGG <- data.frame("Gene ID"=res.df$entrez,
                             "Rank"=res.df$t)
...
gene.list.KEGG <- gene.list.KEGG[!is.na(gene.list.KEGG) & !is.na(names(gene.list.KEGG))]
...
# Add gene symbols to GO BP results, convert to data frame
